...
## 7. 当前未闭合模块的反推结果（v0.2）
### 7.1 Accounts Receivable 链（高置信度）
基于 `109` 数值反推，得到以下稳定关系：

1. `Accounts Receivable-Incurred = - General Conditions fee-Company`（逐期完全一致）。
2. `Accounts Receivable-Company` 在“有审计值的期间”取 `Accounts Receivable-Audited`，无审计值的期间取 `Accounts Receivable-Incurred`。
3. `Accounts Receivable` 行表现为对 `Accounts Receivable-Company` 的累计滚动（running total）。

可执行候选公式：
1. `AR_company_t = IF(AR_audited_t 非空, AR_audited_t, AR_incurred_t)`
2. `AR_balance_t = 累计(AR_company_<=t)`

说明：`Draw Invoice`、`AR-Payment` 在当前快照未体现直接可重复闭式关系，暂保留为审计/对账辅助线。

### 7.2 Budget Cost Change Order 源（中等置信度）
1. `109` 当前值：`2023=+2,175.810493`，`2024=-69,283.242176`，`2026=0`，Total=`-67,107.431683`。
2. `Change Order Log` 与 `Transfer Log` 可见主要候选代码集中在 `66009101`、`66009104`（Owner/GC Contingency）。
3. 当前快照中这两张日志主数据年份集中在 2025，无法直接与 `109` 的 2023/2024 年列一一回放对齐。

结论：`Budget Cost Change Order` 的“最终取数规则”仍需结合你真实业务口径落锤，但候选优先级已缩小到：
1. 以 `Change Order Log` 的 `Approved Change Orders` 为基础。
2. 结合 `Transfer Log` 的 Contingency 相关 code 做净额修正。
3. 最终入 `109` 时按“当期变动额”落列，再累计入 EAC。

### 7.3 Audited 区映射（中等置信度）
1. `General Conditions fee` 与 `Gross Profit` 的主展示行目前更接近 Company 口径，不直接等于 Company+Audited。
2. `Cost of Goods Sold` 的 Total 呈现为 Company+Audited 汇总，但期间列更多体现 Company期值。
3. 这说明“审计区”更像平行对照层，而非统一并入主链计算层。

## 8. 下一步执行方式（不改逻辑、只推进成熟度）
1. 先按第7.1将 `AR` 链正式入字典（该部分已具备编码条件）。
2. `Budget Cost Change Order` 先保留“人工输入+日志校核”双轨，待最终口径确定后切自动。
3. 将 `Audited` 区定义为“对照层字段组”，不直接覆盖主链字段。
4. 完成列映射后，进入增量回写与审计日志联调。
...
5. `Accounts Receivable-Incurred = -General Conditions fee-Company` 在有效期间逐项一致。
6. `Accounts Receivable` 行与 `Accounts Receivable-Company` 累计关系在快照中一致。
