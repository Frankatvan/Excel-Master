...
export function isInternalCompanyVendor(vendor: string, registry: string[]): boolean {
  const normalizedVendor = normalizeInternalCompanyName(vendor);
  return registry.some((name) => normalizeInternalCompanyName(name) === normalizedVendor);
}
...
export async function readInternalCompanies() {
